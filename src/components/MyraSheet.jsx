...
const TRIGGER_PREAPPROVAL = [
  ...
  // Devanagari
  'प्री-अप्रूवल','प्री अप्रूवल','गेट पास','डिलीवरी','डिलीवरी','ब्लिंकिट','स्विगी','ज़ोमाटो',
...
const TRIGGER_HELPDESK = [
  ...
  'पार्किंग','लीक','शोर','कीड़े','सीवेज','गंदगी','टिकट','दर्ज',
];
const HIGH_PRIORITY = [
...
function extractTime(text) {
  const t = text.toLowerCase();
  const match = t.match(/(\d{1,2})\s*(am|pm|baje|बजे)/i);
  if (match) return `${match[1]} ${match[2].replace('बजे','AM').toUpperCase()}`;
...
function extractIssueType(text) {
  ...
  if (t.includes('pest') || t.includes('कीड़े')) return 'Pest Control';
...
export default function MyraSheet({ isClosing, onClose }) {
  ...
  function runFlow(text) {
    ...
    if (!type) {
      ...
      addMessage('myra', "Mujhe samajh nahi aaya. Kya aap dobara bol sakte hain? (I didn't understand. Can you say that again?)");
  ...
  function startPreapprovalFlow(text) {
    ...
    const newFlow = { type: 'preapproval', data, step: 0 };
    setFlow(newFlow);
    askNextPreapproval(newFlow);
  }
  ...
  function askNextPreapproval(f) {
    ...
    if (!data.date) {
      setVoiceState('responding');
      addMessage('myra', 'Kab ke liye chahiye?\n(Kal, parso, ya koi aur date?)');
      setFlow({ ...f, step: 'date' });
      setTimeout(() => startListening(), 1000);
    } else if (!data.time) {
      setVoiceState('responding');
      addMessage('myra', 'Kitne baje expected hai?');
      setFlow({ ...f, step: 'time' });
      setTimeout(() => startListening(), 1000);
    } else if (data.handoff === null) {
      setVoiceState('responding');
      addMessage('myra', 'Security ko dena hai ya directly tumhare flat pe deliver karein?');
      setFlow({ ...f, step: 'handoff' });
      setTimeout(() => startListening(), 1000);
    } else {
      showPreapprovalCard(f);
    }
  }
  ...
  function continueFlow(text) {
    ...
    if (f.type === 'preapproval') {
      const updated = { ...f };
      if (f.step === 'date') {
        const date = extractDate(text) || text.trim();
        updated.data = { ...f.data, date };
      } else if (f.step === 'time') {
        const time = extractTime(text) || text.trim();
        updated.data = { ...f.data, time };
      } else if (f.step === 'handoff') {
        const t = text.toLowerCase();
        const handoff = (t.includes('security') || t.includes('gate')) ? 'Leave with security' : 'Direct to flat';
        updated.data = { ...f.data, handoff };
      }
      setFlow(updated);
      askNextPreapproval(updated);
    } else if (f.type === 'helpdesk') {
      const updated = { ...f };
      if (f.step === 'issue') {
        updated.data = { ...f.data, issueType: extractIssueType(text) || text.trim() };
      } else if (f.step === 'location') {
        updated.data = { ...f.data, location: text.trim() };
      }
      setFlow(updated);
  ...
  function showPreapprovalCard(f) {
    setVoiceState('responding');
    addMessage('myra', 'Yeh details sahi hain?');
  ...
  function startHelpdeskFlow(text) {
    ...
    const location = text.toLowerCase().includes('floor') ? (text.match(/(\d+\w*)\s*floor/i)?.[1] + ' floor') : null;
    const priority = isHighPriority(text) ? 'HIGH' : 'MEDIUM';
    const data = { issueType, location, description: text, priority };
    const newFlow = { type: 'helpdesk', data, step: 0 };
    setFlow(newFlow);
    askNextHelpdesk(newFlow);
  }

  function askNextHelpdesk(f) {
    if (!f.data.issueType) {
      setVoiceState('responding');
      addMessage('myra', 'Kya problem hai — lift, paani, bijli, ya kuch aur?');
      setFlow({ ...f, step: 'issue' });
      setTimeout(() => startListening(), 1000);
    } else if (!f.data.location) {
      setVoiceState('responding');
      addMessage('myra', 'Yeh kahan hai — floor number ya specific area batao?');
      setFlow({ ...f, step: 'location' });
      setTimeout(() => startListening(), 1000);
    } else {
      setVoiceState('responding');
      addMessage('myra', 'Samajh gaya. Yeh details sahi hain?');
      setFlow({ ...f, step: 'confirm' });
      setShowCard(true);
    }
  }
  ...
  function handleEdit() {
    setShowCard(false);
    setFlow(f => ({ ...f, step: 0, data: { ...f.data, date: null, time: null } }));
    addMessage('myra', 'Theek hai, dobara batao. Kab ke liye chahiye?');
    setFlow(f => ({ ...f, step: 'date' }));
    ...
    >
      ...
      <div ref={chatRef} style={{ flex: 1, overflowY: 'auto', padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, justifyContent: 'center' }}>
              {DEMO_CHIPS.map((chip, i) => (
                <button key={i} onClick={() => handleChip(chip)} style={{
                  ...
                  fontSize: 11.5, color: '#444', cursor: 'pointer',
        ...
        {messages.map((m, i) => (
          <div key={i} className="animate-fade-in" style={{
            ...
            <div style={{
              maxWidth: '75%',
              background: m.role === 'user' ? '#0f1f3d' : '#f5f5f5',
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: 4, marginBottom: 8, height: 28, alignItems: 'center' }}>
              {[0, 0.12, 0.24, 0.12, 0].map((d, i) => (
                <div key={i} className="animate-wave-bar" style={{
                  width: 4, borderRadius: 2, background: '#C8102E',
                  animationDelay: `${d}s`, height: 6,
                }} />
              ))}
            </div>
            {transcript && (
              <p style={{ fontSize: 13, color: '#888', fontStyle: 'italic', padding: '0 16px' }}>&#34;{transcript}&#34;</p>
            )}
          </div>
        )}
            ...
            <div style={{ background: '#f5f5f5', borderRadius: '16px 16px 16px 4px', padding: '12px 16px', display: 'flex', gap: 5 }}>
              {[0, 0.2, 0.4].map((d, i) => (
                <div key={i} className="bounce-dot" style={{
                  width: 8, height: 8, borderRadius: '50%', background: '#0f1f3d',
                  animationDelay: `${d}s`,
                }} />
              ))}
            </div>
            ...
            {flow?.type === 'preapproval' ? (
              <>
                ...
                <p style={{ fontSize: 12, color: '#888', marginBottom: 16 }}>
                  {flow.data.visitor} · {flow.data.date} {flow.data.time} · {flow.data.handoff}
                </p>
                <div style={{ border: '2px dashed #e0e0e0', borderRadius: 10, padding: '10px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, marginLeft: 16, marginRight: 16 }}>
                  ...
                  <button onClick={() => navigator.clipboard?.writeText(passcode)} style={{ background: 'none', border: 'none', cursor: 'pointer' }}>
                    <Copy size={18} color="#888" />
                  </button>
                </div>
                <div style={{ background: '#fffbeb', borderRadius: 10, padding: '10px 14px', margin: '0 16px 12px', textAlign: 'left' }}>
                  <span style={{ fontSize: 9, color: '#aaa', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Sponsored</span>
                  <p style={{ fontSize: 12, color: '#1a1a2e', fontWeight: 500, margin: '2px 0 4px' }}>NoBroker Pay — Zero fee on maintenance payments</p>
                  <span style={{ fontSize: 11, color: '#C8102E', fontWeight: 600, cursor: 'pointer' }}>Explore →</span>
                </div>
                <button style={{ display: 'block', width: 'calc(100% - 32px)', margin: '0 16px 8px', background: '#0f1f3d', color: 'white', border: 'none', borderRadius: 12, padding: '13px', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
                  Share via WhatsApp
                </button>
              </>
            ) : (
              <>
                ...
                <div style={{ background: '#eff6ff', borderRadius: 10, padding: '10px 14px', margin: '0 16px 12px', textAlign: 'left' }}>
                  ...
                  <span style={{ fontSize: 11, color: '#C8102E', fontWeight: 600, cursor: 'pointer' }}>Book now →</span>
                </div>
              </>
            )}

            <button onClick={handleDone} style={{ display: 'block', width: 'calc(100% - 32px)', margin: '0 auto', background: '#f0f0f0', color: '#555', border: 'none', borderRadius: 12, padding: '13px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
              Done
            </button>
          </div>
        )}
        ...
        <div className="animate-card-up" style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
            <span style={{ fontSize: 13, fontWeight: 600, color: '#1a1a2e' }}>
              {flow.type === 'preapproval' ? 'Create pre-approval' : 'Raise complaint'}
            </span>
            <span style={{
              background: '#e0eaff', color: '#2563eb',
              fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 10,
            }}>
              {flow.type === 'preapproval' ? (flow.data.visitor?.includes('delivery') ? 'Delivery' : flow.data.visitor?.includes('Cab') ? 'Cab' : 'Guest') : (flow.data.issueType || 'Issue')}
            </span>
          </div>
          ...
          {flow.type === 'preapproval' ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 14 }}>
              {[
                ['Visitor', flow.data.visitor],
                ['Date', flow.data.date],
                ['Time', flow.data.time],
                ['Valid for', '2 hours (default)'],
                ['Handoff', flow.data.handoff],
              ].map(([label, value]) => (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 14 }}>
              {[
                ['Category', flow.data.issueType || '—'],
                ['Issue', (flow.data.description?.slice(0, 60) || '') + '...'],
                ['Location', flow.data.location || 'Not specified'],
                ...
                <div key={label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  {label === 'Priority' ? (
                    <span style={{
                      background: value === 'HIGH' ? '#fee2e2' : '#fef3c7',
                      color: value === 'HIGH' ? '#C8102E' : '#d97706',
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <button onClick={handleConfirm} style={{ flex: 1, background: '#C8102E', color: 'white', border: 'none', borderRadius: 10, padding: '11px', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
              {flow.type === 'preapproval' ? 'Confirm' : 'Confirm & Submit'}
            </button>
            <button onClick={handleEdit} style={{ flex: 1, background: '#f0f0f0', color: '#555', border: 'none', borderRadius: 10, padding: '11px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
              Edit
            </button>
          </div>
        </div>
      ...
      {!showSuccess && (
        <div style={{ flexShrink: 0, padding: '8px 16px 12px', borderTop: '1px solid #f0f0f0' }}>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 8 }}>
            <input
              ...
              style={{
                ...
                padding: '10px 16px', fontSize: 13, color: '#1a1a2e',
                outline: 'none',
              }}
            />
          ...
          <p style={{ fontSize: 10, color: '#bbb', textAlign: 'center', letterSpacing: '0.02em' }}>
            English · हिन्दी · ಕನ್ನಡ · தமிழ் · తెలుగు · मराठी
          </p>
        </div>
      )}
    </div>
